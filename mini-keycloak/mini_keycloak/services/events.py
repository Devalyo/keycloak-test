...
EVENT_TYPES = frozenset({
    ...
    'SEND_RESET_PASSWORD', 'UPDATE_PASSWORD', 'UPDATE_CREDENTIAL',
})
...
class EventService:
    ...
    Request details must match finite values. Reset identity details are
    projected only from persisted users and authentication sessions.
    """
    ...
    def record(self, realm_id, event_type, *, client_id=None, user_id=None,
               user_session_id=None, authentication_session_id=None,
               source_address=None, error=None, details=None):
        ...
        if authentication_session_id is not None:
            auth = self.session.get(AuthenticationSession, authentication_session_id)
            user = self.session.get(User, user_id) if user_id is not None else None
            if (auth is None or auth.realm_id != realm_id or auth.client_id != client_id
                    or user is None or auth.selected_user_id != user.id):
                raise ValueError('Invalid event identity')
            if event_type in {'SEND_RESET_PASSWORD', 'UPDATE_PASSWORD', 'UPDATE_CREDENTIAL'}:
                safe_details.update(code_id=auth.tab_id[:128], username=user.username[:255])
                if user.email:
                    safe_details['email'] = user.email[:320]
