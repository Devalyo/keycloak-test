...
def link_href(response_text: str, path: str) -> str:
    links = re.findall(r'<a[^>]+href=["\']([^"\']+)["\']', response_text, re.IGNORECASE)
    for href in links:
        candidate = unescape(href)
        if path in urlparse(candidate).path:
            return candidate
    raise AssertionError(f"no link href contains {path!r}: {links!r}")
