...
LOGIN_ACTIONS_PATH = '/realms/demo/login-actions/'
...
def test_authorization_issues_opaque_scoped_preauth_cookie(app, client, secure):
    ...
    assert cookie['path'] == LOGIN_ACTIONS_PATH and not cookie['domain']
...
@pytest.mark.parametrize('binding', [
    'missing', 'tampered', 'non-ascii', 'other-tab', 'other-realm'])
def test_invalid_browser_binding_rejected_before_credentials(app, client, monkeypatch, binding):
    ...
    elif binding == 'non-ascii':
        client.set_cookie(cookie.key, 'é', path=cookie['path'])
    else:
...
def test_invalid_credentials_preserve_binding_for_retry(client):
    ...
    assert cookie is not None
    action = form_action(page.text, 'login-actions/authenticate')
    rejected = client.post(action, data=CREDENTIALS | {'password': 'incorrect'})
    assert rejected.status_code == 401
    assert client.get_cookie(cookie.key, path=cookie['path']).value == cookie.value
    retry = form_action(rejected.text, 'login-actions/authenticate')
    assert query_value(retry, 'session_code') != query_value(action, 'session_code')
    assert client.post(retry, data=CREDENTIALS).status_code == 302
...
def test_concurrent_login_completions_commit_one_session_and_rollback_loser(app, client, monkeypatch):
    ...
    assert len(set(attempted_sids)) == 1
